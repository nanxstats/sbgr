...
or [create an issue on GitHub](https://github.com/nanxstats/sbgr/issues)
...
To get your authentication token, please [login](https://accounts.sbgenomics.com/)
to the Seven Bridges Platform. Go to the _Developer - Auth Token_ tab,
then click the button to generate the auth token.
...
# Create new project
sbgr_project = project_new(
  token, name = "sbgr tutorial",
...
# download the SBG CLI uploader to home directory
misc_get_uploader('~/')

# suppose we have extracted sample1.fastq into current R working directory
file_id = misc_upload_cli(
  token, project_id = sbgr_project_id,
...
pipeline_add(
  token, project_id_to = sbgr_project_id,
  pipeline_id = pipeline_mat[which(pipeline_mat[, 2] == "FastQC Analysis"), 1])
```
...
fastqc_details = pipeline_details(
  token, project_id = sbgr_project_id, pipeline_id = pipeline_id)
...
report_url = file_download_url(
  token, project_id = sbgr_project_id,
  file_id = task_running[["outputs"]][[1]][[1]])
# download the output archive using wget
download.file(
  url = report_url[["url"]], destfile = "fastqc_report.zip", method = "wget")
